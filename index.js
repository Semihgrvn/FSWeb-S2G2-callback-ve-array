...
//  const final2014 = fifaData.filter(item => {return item.Stage === "Final" && item.Year === 2014});
...
function OrtalamaGolSayisi(indeks) {
    const toplamGol = indeks.reduce((toplam,mac)=> {
        return toplam + (mac["Home Team Goals"]+ mac["Away Team Goals"])
    },0);
    let ort = toplamGol/indeks.length;
